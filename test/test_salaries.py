...
def test_read_salaries(client: TestClient):
    ...
    client.post("/users", json={"email": "testuser@example.com", "password": "password"})
    response = client.post("/tokens", json={"email": "testuser@example.com", "password": "password"})
    ...
    response = client.get("/salaries", headers={"Authorization": token_data["access_token"]})
